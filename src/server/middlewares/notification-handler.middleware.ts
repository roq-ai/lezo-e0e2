...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.update': {
    roles: ['delivery-driver', 'business-owner', 'restaurant-manager'],
    key: 'order-status-update',
    tenantPath: ['organization', 'order'],
    userPath: [],
  },
  'menu.update': {
  ...
  'complaint.update': {
    roles: ['customer-service-representative', 'business-owner'],
    key: 'complaint-status-update',
    tenantPath: ['organization', 'order', 'complaint'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['delivery-driver'];
